...
function apiErrorMessage(res, fallback = "Request failed") {
...
function clearBeforeUnloadAndClose(tabId) {
  if (!tabId) return;
...
function scheduleTabClose(tabId, delayMs = CLOSE_AFTER_SUBMIT_DELAY_MS) {
  if (!tabId) return;
  const safeDelay = Number.isFinite(delayMs) ? Math.max(0, delayMs) : CLOSE_AFTER_SUBMIT_DELAY_MS;
  setTimeout(() => clearBeforeUnloadAndClose(tabId), safeDelay);
}
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  chrome.scripting.executeScript({
    target: { tabId },
    args: [CLOSE_AFTER_SUBMIT_DELAY_MS],
    func: (closeDelayMs) => {
      ...
      function submitAndClose() {
        ...
        chrome.runtime.sendMessage({ type: "SCHEDULE_CLOSE_TAB", delayMs: closeDelayMs }).catch(() => {
          setTimeout(() => chrome.runtime.sendMessage({ type: "CLOSE_TAB" }).catch(() => {}), closeDelayMs);
        });
        return true;
      }
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === "SCHEDULE_CLOSE_TAB") {
    scheduleTabClose(sender.tab?.id, msg.delayMs);
    sendResponse({ ok: true });
    return true;
  }

  if (msg.type === "CLOSE_TAB") {
    clearBeforeUnloadAndClose(sender.tab?.id);
    sendResponse({ ok: true });
    return true;
  }
