...
function detectJobOnPage() {
  ...
  if (h.includes("linkedin.com")) {
    ...
    // Widened selector list (SPA job portals like JobDiva/Ceipal use non-standard class
    // names) plus a full-page-text fallback so a real job page doesn't save with an
    // empty description just because none of the known selectors matched.
    description =
      firstText([
    ...
    if (!pathLooksJobLike || description.length < MIN_DESCRIPTION_LENGTH) title = "";
  }
