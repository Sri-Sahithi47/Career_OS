...
(async () => {
  ...
  function jobForSave(job) {
    ...
    return {
      ...job,
      url,
      title: cleanText(job?.title) || document.title || "Untitled role",
  ...
  function saveErrorLabel(response) {
    const detail = cleanText(response?.error);
    if (response?.status === 422) return "Missing job info";
    if (response?.status === 403) return "Sign in again";
    if (response?.status === 401 || detail === "Not authenticated") return "Sign in first";
    return "Failed — retry";
  }

  function saveErrorDetail(response) {
    const detail = cleanText(response?.error);
    if (response?.status === 422) return detail || "This page is missing required job details.";
    if (response?.status === 403) return detail || "Your session needs a refresh. Sign in from the extension popup again.";
    if (response?.status === 401 || detail === "Not authenticated") return "Sign in from the extension popup, then save again.";
    return detail || "Save failed. Try again.";
  }
  ...
  function linkedInDescription(root = document) {
    const selectors = [
      "#job-details",
  ...
  function injectPill(job) {
    ...
    button.addEventListener("click", async () => {
      ...
      if (response?.ok) {
        ...
        status.textContent = saveErrorDetail(response);
        button.textContent = saveErrorLabel(response);
        button.disabled = false;
      }
    });
  }
  ...
  function injectLauncher(job) {
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      if (response?.ok) {
        ...
        saveBtn.querySelector(".careeros-launcher-bar-label").textContent = saveErrorLabel(response);
        saveBtn.title = saveErrorDetail(response);
        saveBtn.disabled = false;
      }
    });
  ...
  function injectZipFilterBar() {
    ...
    bar.querySelector("[data-careeros-zip-save]").addEventListener("click", async (e) => {
      ...
      if (response?.ok) {
        ...
        saveBtn.textContent = saveErrorLabel(response).replace("—", "-");
        saveBtn.title = saveErrorDetail(response);
        saveBtn.disabled = false;
      }
    });
  ...
  // (e.g. an email subject mentioning "Developer" would otherwise pass the title heuristic).
  const NON_JOB_HOSTS = /(^|\.)(mail\.google\.com|calendar\.google\.com|docs\.google\.com|drive\.google\.com|outlook\.office\.com|outlook\.live\.com|outlook\.office365\.com)$/i;

  const MIN_DESCRIPTION_LENGTH = 120;

  function parseGeneric() {
    if (NON_JOB_HOSTS.test(window.location.hostname)) return null;
    ...
    const location =
      ...
      "";
    // Widened selector list (SPA job portals like JobDiva/Ceipal use non-standard
    // class names), plus a full-page-text fallback so a real job page never saves
    // with an empty description just because none of the known selectors matched.
    const description =
      firstText([
        '[class*="description" i]', '[data-testid*="description" i]', '[data-automation-id*="description" i]',
        '[id*="description" i]', '[class*="jobdetail" i]', '[id*="jobdetail" i]',
    ...
    if (!title || !pathLooksJobLike || description.length < MIN_DESCRIPTION_LENGTH) return null;
    return {
      title,
      company,
      location,
