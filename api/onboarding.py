...
async def extract_text_from_file(file: UploadFile) -> str:
    ...
    if filename.endswith(".pdf"):
        try:
            ...
            if not text.strip():
                raise HTTPException(status_code=400, detail="This PDF has no readable text. Upload a text-based PDF or paste your resume text.")
            return text.strip()
    ...
    if filename.endswith(".doc"):
        ...
        try:
            ...
            if result.returncode != 0 or not result.stdout.strip():
                raise HTTPException(
                    status_code=400,
                    detail="Could not read this DOC file. Save it as DOCX or PDF, or paste the resume text.",
                )
            return result.stdout.strip()
        except FileNotFoundError:
            raise HTTPException(
                status_code=400,
                detail="DOC upload needs macOS textutil on this machine. Save as DOCX or PDF, or paste the resume text.",
            )
        except subprocess.TimeoutExpired:
            raise HTTPException(status_code=400, detail="Reading this DOC file timed out. Save it as DOCX or PDF and try again.")
        finally:
            try:
                os.unlink(tmp_path)
            except OSError:
