...
# Spreadsheet cleanup is an explicit maintenance operation, never a startup side effect.
...
@app.post("/api/update-analysis", dependencies=[Depends(reject_legacy_endpoint)])
...
@app.post("/api/ai-tailor-resume", dependencies=[Depends(reject_legacy_endpoint)])
...
@app.post("/api/generate-resume", dependencies=[Depends(reject_legacy_endpoint)])
...
def download_resume(filename: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    owned_paths = {
        row.workspace_resume_path
        for row in db.query(MatchedJob).filter(MatchedJob.user_id == user.id).all()
        if row.workspace_resume_path
    }
    events = db.query(ApplicationEvent).join(MatchedJob).filter(
        MatchedJob.user_id == user.id,
        ApplicationEvent.event_type == "resume_generated",
    ).all()
    owned_paths.update((event.metadata_json or {}).get("pdf_path") for event in events)
    if not any(Path(path).resolve() == pdf_path for path in owned_paths if path):
        raise HTTPException(status_code=404, detail="PDF not found")
...
@app.delete("/api/delete-job/{row_index}", dependencies=[Depends(reject_legacy_endpoint)])
...
@app.post("/api/backup", dependencies=[Depends(reject_legacy_endpoint)])
def create_backup():
...
@app.get("/api/backups", dependencies=[Depends(reject_legacy_endpoint)])
def list_backups():
