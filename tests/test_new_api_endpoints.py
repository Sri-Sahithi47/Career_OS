...
These tests use an in-process API, temporary database, and mocked AI service.
Run: python -m pytest tests/test_new_api_endpoints.py -v
"""
...
def isolated_api(tmp_path_factory):
    ...
    from api import auth, onboarding, jobs, internal
    from api.deps import get_db
    from src.database import Base
    from src.settings import settings

    directory = tmp_path_factory.mktemp('endpoint-tests')
    engine = create_engine(f"sqlite:///{directory / 'tests.db'}", connect_args={"check_same_thread": False})
    ...
    app = FastAPI()
    for router in (auth.router, onboarding.router, jobs.router, internal.router):
        app.include_router(router)
    app.dependency_overrides[get_db] = get_test_db
    with pytest.MonkeyPatch.context() as mp, TestClient(app) as client:
        mp.setattr(settings, 'RESUMES_DIR', directory / 'resumes')
        mp.setattr(settings, 'INTERNAL_API_TOKEN', INTERNAL_TOKEN)
        mp.setattr(settings, 'AI_MATCH_ENABLED', False)
        mp.setattr(settings, 'LEGACY_DELIVERY_FALLBACK_ENABLED', False)
        mp.setattr(jobs, 'generate_tailored_resume_data', lambda *args, **kwargs: {
            'ats_score': 80, 'location': 'Remote', 'tech_stack': {'Languages': ['Python']}, 'points': ['Built Python services'],
        })
        ...
        mp.setattr(sys.modules[__name__], 'requests', RequestsAdapter())
        yield
    engine.dispose()
...
def job_id(authed_user):
    ...
    if jid is None:
        pytest.fail("Synthetic job was not delivered into the isolated test database")
    return jid
...
class TestTailorJobWorkspace:
    ...
    def test_tailor_with_mocked_groq_succeeds(self, token, job_id):
        ...
        with patch("api.jobs.generate_tailored_resume_data", return_value=mock_tailored):
            r = requests.post(
                f"{API}/api/jobs/{job_id}/tailor",
                headers=_auth_headers(token),
            )
...
class TestGenerateJobResume:
    ...
    def test_generate_resume_with_mocked_workspace_service_succeeds(self, token, job_id):
        ...
        with patch("src.resume.pdf_export.export_resume_pdf", return_value=(mock_path, mock_url)):
            r = requests.post(
                f"{API}/api/jobs/{job_id}/resume",
                headers=_auth_headers(token),
            )
