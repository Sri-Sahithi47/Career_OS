...
def _alias_variants(term: str) -> set[str]:
    """All alias forms that count as the same skill as `term` (including itself)."""
    canonical = _to_canonical(term)
    for group in _ALIAS_GROUPS:
        if canonical in group or term.lower() in group:
            return group
    return {term.lower()}


def count_term_occurrences(term: str, text: str) -> int:
    """How many times `term` (or an alias of it, e.g. AWS ↔ Amazon Web Services) appears in text."""
    if not text:
        return 0
    lowered = text.lower()
    total = 0
...
_ENV_LINE_RE = re.compile(r'Environment\s*:\s*([^\n]+)', re.IGNORECASE)
_RESPONSIBILITIES_RE = re.compile(r'Responsibilit(?:y|ies)\s*:?', re.IGNORECASE)
_JUNK_LINE_RE = re.compile(r'^(top|bottom) of form$', re.IGNORECASE)


def analyze_client_environments(resume_text: str) -> list[dict]:
    """
    Parse an IT-consulting-style resume into per-client blocks of the form:
        <Client / role header>
        Responsibilities:
        <bullet points...>
        Environment: TechA, TechB, TechC

    For each block, flag which Environment-listed technologies are never mentioned anywhere
    in that client's Responsibilities bullets — i.e. claimed in the tech-stack line but never
    actually described as having been used.
    """
    text = resume_text or ""
    blocks = []
    matches = list(_ENV_LINE_RE.finditer(text))
    prev_end = 0
    for idx, m in enumerate(matches):
        segment = text[prev_end:m.start()]
        prev_end = m.end()

        resp_match = _RESPONSIBILITIES_RE.search(segment)
        header_text = segment[:resp_match.start()] if resp_match else segment
        responsibilities_text = segment[resp_match.end():] if resp_match else segment

        # The client/role header sits on the last line(s) right before "Responsibilities:" —
        # take those rather than the first non-empty line, which for the first block would be
        # the resume's own name/summary header instead of the actual employer.
        header_lines = [
            line.strip() for line in header_text.splitlines()
            if line.strip() and not _JUNK_LINE_RE.match(line.strip())
        ]
        if len(header_lines) >= 2:
            client_label = " — ".join(header_lines[-2:])
        elif header_lines:
            client_label = header_lines[-1]
        else:
            client_label = f"Role {idx + 1}"
        client_label = re.sub(r'\s+', ' ', client_label).strip()

        env_keywords = sorted(scan_whitelist(m.group(1)))
        resp_canonical = {_to_canonical(k) for k in scan_whitelist(responsibilities_text)}
        not_demonstrated = [kw for kw in env_keywords if _to_canonical(kw) not in resp_canonical]

        blocks.append({
            "client": client_label[:120],
            "environment_keywords": env_keywords,
            "not_in_responsibilities": not_demonstrated,
        })
    return blocks


_NON_TECHNICAL_CATEGORIES = {'soft skills'}
...
def _load_whitelist() -> list[str]:
    ...
    for line in path.read_text().splitlines():
        ...
        if not line:
            continue
        if line.startswith('#'):
            header = line.lstrip('#').replace('─', '').strip()
            if header:
                category = header
            continue
        skills.append(line)
        _skill_category[line.lower()] = category
    return skills


def is_technical(term: str) -> bool:
    """Whether a whitelist-scanned keyword falls outside the Soft Skills category."""
    _get_whitelist()
    category = _skill_category.get(term.lower(), '')
    return category.lower() not in _NON_TECHNICAL_CATEGORIES
