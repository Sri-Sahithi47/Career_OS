...
class TailorServiceUnavailable(TailorServiceError):
    pass
...
Rules:
- Use only skills and experience explicitly supported by the candidate's resume.
- Never invent employers, degrees, achievements, metrics, or qualifications.
...
def generate_tailored_resume_data(
    ...
    *,
    ...
    if not hasattr(settings, 'GROQ_API_KEYS') or not settings.GROQ_API_KEYS:
        raise TailorServiceUnavailable("GROQ_API_KEYS not configured in settings")
    ...
    if cache_path.exists():
        try:
            return TailoredResumeData.model_validate_json(cache_path.read_text()).model_dump()
    ...
    for model_name in models_to_try:
        ...
        for api_key in keys_to_try:
            try:
                ...
                tailored_data = TailoredResumeData.model_validate(tailored_data).model_dump()
                ...
                print(f"[tailor_service] Request failed on model {model_name}: {type(e).__name__}")
                last_exception = e
                error_str = str(e).lower()
                if isinstance(e, (ValidationError, ValueError)) or any(marker in error_str for marker in ("429", "rate limit", "401", "json", "extract", "timeout", "timed out", "503")):
