...
import { normalizeStatus } from './lib/jobs'
...
const NAV_ICONS = {
  ...
  resumeCheck: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h8"/><polyline points="14 2 14 8 20 8"/><circle cx="17" cy="17" r="3"/><line x1="19.5" y1="19.5" x2="22" y2="22"/></svg>,
...
function App() {
  ...
  const loadApp = async () => {
    ...
    try {
      ...
      if (version === workspaceVersion.current) setLoading(false)
    }
  }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      if (refreshing || pendingWrites.current) return
      refreshing = true
      const version = workspaceVersion.current
      try {
        const response = await api.get('/api/jobs', { params: { limit: JOB_FETCH_LIMIT } })
        if (disposed || version !== workspaceVersion.current || pendingWrites.current) return
  ...
  const handleNotesChange = async (job, notes) => {
    ...
    try {
      ...
      setJobs((currentJobs) => currentJobs.map((currentJob) => currentJob.id === job.id ? { ...currentJob, Notes: notes } : currentJob))
  ...
  const savedSearches = onboarding?.search_presets || []
    ...
    <div className={`app-frame${sidebarCollapsed ? ' sidebar-collapsed' : ''}`}>
      ...
      <aside className="app-sidebar">
        ...
        <nav className="topnav" aria-label="Tools">
          ...
          <NavLink to="/resume-check" className={({ isActive }) => `topnav-link ${isActive ? 'active' : ''}`}>
            <span className="nav-icon">{NAV_ICONS.resumeCheck}</span>
            <span className="nav-label">Resume Check</span>
          </NavLink>
        </nav>
