...
function TailorView({ jobs, setView, tailoredData, setTailoredData, selectedJobId, setSelectedJobId, onboarding }) {
  ...
  const [jd, setJd] = useState(() => selectedJob?.['Job Description'] || selectedJob?.Description || '')
  const baseResume = onboarding
  ...
  const handleGenerate = async () => {
    ...
    if (!hasResume) { setErrorMsg('Upload a resume in Settings before tailoring.'); return }
    if (!jd.trim()) { setErrorMsg('Enter a job description before tailoring.'); return }
    setTailorStep(0)
    setLoadingTailor(true)
    setErrorMsg('')
    try {
      const resp = await api.post(`/api/jobs/${selectedJobId}/tailor`, { job_description: jd.trim() }, { timeout: 180000 })
      setTailoredData(resp.data.tailored_data || {})
      setView('preview')
    } catch (e) {
      const detail = getApiErrorMessage(e, 'Tailor request failed. Try again.')
    ...
    <>
      ...
      {!hasResume && (
        <div className="tp-banner tp-banner-warn">
          ...
          Upload a resume before tailoring. <a href="/settings" style={{ color: '#b45309', fontWeight: 600 }}>Open Settings →</a>
        </div>
      )}
...
export function PreviewView({ setView, tailoredData, setTailoredData, selectedJob }) {
  ...
  const saved = savedData === tailoredData
  ...
  const persistWorkspace = () => api.patch(`/api/jobs/${selectedJob.id}/analysis`, tailoredData)
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      setSavedData(tailoredData)
      const resp = await api.post(`/api/jobs/${selectedJob.id}/resume`, null, { timeout: 180000 })
      setPdfUrl(`${API_URL}${resp.data.pdf_url}`)
    } catch (e) {
      setPdfError(getApiErrorMessage(e, 'PDF generation failed.'))
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setErrorMsg(getApiErrorMessage(e, 'Could not save resume changes.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <>
      <header className="tp-topbar">
        ...
        <div className="tp-topbar-actions">
          <button type="button" className="tp-btn-ghost" onClick={handleGeneratePdf} disabled={generatingPdf || !selectedJob?.id}>
            ...
            {generatingPdf ? 'Generating PDF…' : pdfUrl ? 'Regenerate PDF' : 'Generate PDF'}
          </button>
          {pdfUrl && (
          ...
          <button type="button" className="tp-btn-primary" onClick={handleSave} disabled={saved || saving || generatingPdf}>
            {saving ? 'Saving…' : saved ? 'Saved ✓' : 'Save to Job Card'}
          </button>
        </div>
      </header>
      ...
      {generatingPdf && (
        <div className="tp-loading-steps">
          <div className="tp-step active"><span className="tp-step-dot" />Generating your resume PDF…</div>
          <span className="tp-muted" style={{ fontSize: 12 }}>Preparing download</span>
        </div>
      )}
...
function TailorPage({ jobs = [], onboarding }) {
  ...
  const selectJob = (id) => {
    setSelectedJobId(id)
    setTailoredData(jobs.find((job) => String(job.id || getJobId(job)) === id)?.['Analysis Data'] || null)
  }
