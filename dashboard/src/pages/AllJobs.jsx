...
function getConfidenceLabel(score) {
  if (!score) return { label: 'Not scored', cls: 'conf-low' }
...
function NotesEditor({ job, onNotesChange }) {
  ...
  const save = async () => {
    if (!dirty || saving || !onNotesChange) return
    setSaving(true)
    try {
      if (await onNotesChange(job, value) !== false) setSavedValue(value)
    } finally {
      setSaving(false)
    }
  }
...
export function DetailPanel({ job, onNotesChange }) {
  ...
  const jobType    = job['Job Type'] || job['Employment Type'] || job.remote || ''
      ...
      <div className="detail-scroll-body">
        ...
        {activeTab === 'Job Description' && (
          <div className="detail-body">
            ...
            {score > 0 && <section className="detail-fit-card">
              <span className="detail-section-label">Profile fit</span>
              <h3 className="detail-fit-title">Match details</h3>
        ...
        {activeTab === 'My Notes' && (
          <div className="detail-body">
            <section className="detail-section">
              ...
              <NotesEditor key={job.id} job={job} onNotesChange={onNotesChange} />
            </section>
          </div>
        )}
...
const AllJobs = ({ jobs, onNotesChange, onJobAdded }) => {
  ...
  const setSearchQuery = (query) => setSearchParams((current) => { current.set('q', query); return current }, { replace: true })
  ...
  return (
    <section className="alljobs-page">
      <header className="alljobs-header">
        ...
        <div className="alljobs-toolbar">
          ...
          <div className="alljobs-toolbar-actions">
            <button type="button" className="toolbar-more-btn" onClick={() => setAddingJob(true)}>
              Add Job
            </button>
      ...
      {addingJob && <AddJobDialog onClose={() => setAddingJob(false)} onSaved={(job) => {
        onJobAdded?.(job)
        setSearchQuery('')
        setActiveFilters([])
        setRoleFilter('Any')
