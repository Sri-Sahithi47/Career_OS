...
function OnboardingPage({ onboarding, onCompleted, onUpdated }) {
  ...
  const canContinue =
    step === 'welcome' ||
    (step === 'resume' && (pasteMode ? pastedResume.trim().length >= 50 : resumeFile !== null || resumeFileName) && termsAccepted) ||
  ...
  const saveResume = async () => {
    let payload
    if (pasteMode) {
      payload = { resume_text: pastedResume.trim(), filename: 'resume.txt', content_type: 'text/plain' }
    } else {
      if (!resumeFile) return Boolean(resumeFileName)
      payload = new FormData()
      payload.append('file', resumeFile)
    }
    const response = await api.post('/api/onboarding/resume', payload)
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(requestError, 'Unable to save this step right now.'))
    } finally {
      setSubmitting(false)
    }
  }
...
function RolesStep({ profile, setProfile, inferredSkills, customRole, setCustomRole, addCustomRole }) {
