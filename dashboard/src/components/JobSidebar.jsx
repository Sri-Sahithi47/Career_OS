...
function JobSidebar({ job, onClose, onStatusChange, onDelete, onNext, onPrev, hasNext, hasPrev }) {
  ...
  const handleAutoTailor = async () => {
    ...
    try {
      const response = await api.post(`/api/jobs/${job.id}/tailor`, null, { timeout: 180000 })
  ...
  const handleGenerateResume = async () => {
    ...
    try {
      const data = jsonMode ? JSON.parse(jsonText) : editedData
      await api.patch(`/api/jobs/${job.id}/analysis`, {
        ats_score: data.ats_score !== 'N/A' ? Number(data.ats_score) : null,
        location: data.location || '',
        tech_stack: data.tech_stack || {},
        suggested_tech_stack: data.suggested_tech_stack || {},
        points: data.points || [],
      })
      setOriginalData(data)
      const response = await api.post(`/api/jobs/${job.id}/resume`, null, { timeout: 180000 })
